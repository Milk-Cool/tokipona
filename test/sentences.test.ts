...
const testValidity = (sent: Sentence, val: boolean, valExpected: [boolean, boolean, boolean, boolean?, boolean?, boolean?]) => {
    ...
    if(valExpected[0]) expect(sent.subject).toBeDefined();
    ...
    if(valExpected[2]) expect((sent.actions as Action[])[0].object).toBeDefined();
...
test("correctly parses simple questions", () => {
    ...
    const [res1, _rem1, val1] = nextSentence(test1);
    testValidity(res1, val1, [true, true, true, true]);
    expect(joinNoun(res1.subject as Noun)).toBe("sina");
    expect(joinVerb((res1.actions as Action[])[0].verb as Verb)).toBe("moku ala moku");
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("kili");
    expect(joinTime(res1.time as Time)).toBe("ni");

    const test2 = "sina pona ala pona?";
    const [res2, _rem2, val2] = nextSentence(test2);
    testValidity(res2, val2, [true, false, false]);
    expect(joinNoun(res2.subject as Noun)).toBe("sina pona ala pona");
});
...
test("correctly parses multiple sentences", () => {
    ...
    testValidity(res1, val1, [true, true, true]);
    expect(joinNoun(res1.subject as Noun)).toBe("jan seme");
    expect(joinVerb((res1.actions as Action[])[0].verb as Verb)).toBe("pakala");
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("ona");
    [res1, test1, val1] = nextSentence(test1);
    testValidity(res1, val1, [true, true, true]);
    expect(joinNoun(res1.subject as Noun)).toBe("jan lili");
    expect(joinVerb((res1.actions as Action[])[0].verb as Verb)).toBe("pakala");
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("ona");
    [res1, test1, val1] = nextSentence(test1);
...
test("correctly parses complex sentences (sentence la sentence)", () => {
    ...
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("kili");
    expect(joinTime(res1.time as Time)).toBe("ni");
    expect(joinNoun((res1.la as Sentence).subject as Noun)).toBe("mi pilin pona");
    expect(joinNoun(((res1.la as Sentence).la as Sentence).subject as Noun)).toBe("mi pilin ike ala");
    ...
    expect(joinNoun(res2.subject as Noun)).toBe("jan lili");
    expect(joinVerb((res2.actions as Action[])[0].verb as Verb)).toBe("pona");
    expect(joinNoun((res2.la as Sentence).subject as Noun)).toBe("jan lili");
    expect(joinVerb(((res2.la as Sentence).actions as Action[])[0].verb as Verb)).toBe("wile moku");
    expect(joinNoun(((res2.la as Sentence).actions as Action[])[0].object as Noun)).toBe("moku");
});
...
test("correctly parses sentences with multiple subjects separated with anu and en", () => {
    ...
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("toki pona en toki Inli");
    ...
    expect(joinNoun((res2.actions as Action[])[0].object as Noun)).toBe("kili anu suwi");
});
...
test("correctly parses sentences with multiple actions separated with li", () => {
    ...
    expect(joinNoun(res1.subject as Noun)).toBe("mi");
    expect(joinVerb((res1.actions as Action[])[0].verb as Verb)).toBe("toki");
    expect(joinNoun((res1.actions as Action[])[0].object as Noun)).toBe("toki pona");
    expect(joinVerb((res1.actions as Action[])[1].verb as Verb)).toBe("olin");
    expect(joinNoun((res1.actions as Action[])[1].object as Noun)).toBe("jan olin mi");
    ...
    expect(joinNoun(res2.subject as Noun)).toBe("jan lili");
    expect(joinVerb((res2.actions as Action[])[0].verb as Verb)).toBe("wile");
    expect(joinNoun((res2.actions as Action[])[0].object as Noun)).toBe("kili");
    expect(joinVerb((res2.actions as Action[])[1].verb as Verb)).toBe("wile");
    expect(joinNoun((res2.actions as Action[])[1].object as Noun)).toBe("suwi");
});
...
test("correctly parses sentences with tan", () => {
    ...
    expect(joinNoun(res1.subject as Noun)).toBe("sina pilin ike");
    expect(joinNoun((res1.tan as Sentence).subject as Noun)).toBe("seme");
